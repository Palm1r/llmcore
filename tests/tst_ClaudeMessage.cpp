...
TEST(ClaudeMessage, HandleTextBlock)
{
    ...
    EXPECT_TRUE(std::holds_alternative<TextContent>(msg.getCurrentBlocks()[0]));
...
TEST(ClaudeMessage, HandleImageBlock)
{
    ...
    EXPECT_EQ(msg.getCurrentBlocks().size(), 1);
    auto *imgBlock = std::get_if<ImageContent>(&msg.getCurrentBlocks()[0]);
...
TEST(ClaudeMessage, HandleMixedContent_TextImageToolUse)
{
    ...
    EXPECT_TRUE(std::holds_alternative<TextContent>(msg.getCurrentBlocks()[0]));
    EXPECT_TRUE(std::holds_alternative<ImageContent>(msg.getCurrentBlocks()[1]));
    EXPECT_TRUE(std::holds_alternative<ToolUseContent>(msg.getCurrentBlocks()[2]));
}
