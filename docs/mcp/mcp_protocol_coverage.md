...
|---|---|---|
| Request with `id` (string or integer) | ✅ | `JsonRpcSession` allocates string ids from `QAtomicInteger` (`source/rpc/JsonRpcSession.cpp`). Strings, not integers, to avoid edge cases. |
...
| Response echoes request `id` | ✅ | `JsonRpcSession::sendResponse`. |
| Error envelope `{ code, message, data? }` | ✅ | `JsonRpcSession::sendError`. |
| Notifications (no `id`) | ✅ | `JsonRpcSession::sendNotification`. |
...
| MCP-extension error codes (-32002, -32800, -32801) | ✅ | `-32800 RequestCancelled` actively returned by `dispatchRequest` when the handler's future throws `Rpc::CancelledError`. |
| Server throws `Rpc::RemoteError` → serialised to error envelope | ✅ | `source/rpc/JsonRpcSession.cpp`. |
...
|---|---|---|
| Newline-delimited JSON framing | ✅ `include/LLMQore/RpcLineFramer.hpp` | ✅ same |
| UTF-8, compact JSON, one object per line | ✅ | ✅ |
| Stderr reserved for logging | ✅ | ✅ |
| `QProcess` launch on client, with env + cwd + startup timeout | ✅ `Rpc::StdioClientTransport` | n/a |
...
| Feature | Status |
|---|---|
| `Rpc::PipeTransport::createPair` for loopback tests and embedded server+client | ✅ |
...
throwing `Rpc::RemoteError(ErrorCode::InvalidParams, ...)` from inside the future.
...
| Scope | Where |
|---|---|
| `Rpc::LineFramer` (partial reads, CRLF, UTF-8 boundaries, empty lines) | `tests/tst_RpcLineFramer.cpp` (7 cases) |
...
`McpServer` over `Rpc::PipeTransport`, covering tool discovery
